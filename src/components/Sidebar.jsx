  ...
  const NavLink = ({ el }) => (
    <Link
      ...
      className={clsx(
        ...
        path === el.link.split('/')[0] ? 'bg-fuchsia-800 text-neutral-100' : ''
      )}
      aria-label={`Navigate to ${el.label}`}
    >
      {el.icon}
      <span className="hover:text-fuchsia-800">{el.label}</span>
    </Link>
  );
  ...
  return (
    <div className="w-full h-full flex flex-col gap-6 p-5">
      <h1 className="flex gap-1 items-center">
        <p className="p-2 rounded-full bg-fuchsia-800">
          <MdOutlineAddTask className="text-white text-2xl font-black" />
        </p>
        <span className="text-2xl font-bold text-fuchsia-800">Taskky</span>
      </h1>
